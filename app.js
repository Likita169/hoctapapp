...
function renderTabbar(){
  ...
  const tabs = [
    ...
    {id:'vocab', icon:'🔤', label:'Từ vựng Anh'},
...
/* 27-tu-vung-anh.js — Tab "Từ vựng Anh": kho từ vựng tiếng Anh RIÊNG, tách hẳn khỏi
   cây bộ thẻ ở Trang chủ (không lẫn vào DATA.cards/subjects, có tab + màn hình
   riêng). Vẫn ôn theo đúng thuật toán lặp lại ngắt quãng và dùng chung
   grade()/recordXpAndStreak() với thẻ thường, nên ôn từ vựng vẫn cộng chung
   XP/streak/huy hiệu với phần còn lại của app. */

/* ---------------- dữ liệu + tiện ích ---------------- */
// Đảm bảo DATA.vocab luôn là mảng và mỗi từ có đủ trường SRS mặc định — kể cả
...
function normalizeVocab(){
  ...
  DATA.vocab.forEach(w=>{
    ...
    if(w.reps===undefined) w.reps = 0;
    if(w.due===undefined) w.due = Date.now();
...
function renderVocabHome(){
  ...
  const header = document.createElement('header');
  header.className = 'topbar';
  header.innerHTML = `<h1 class="display">Từ vựng Anh</h1>`;
  const headerActions = document.createElement('div');
  headerActions.style.cssText = 'display:flex; align-items:center; gap:12px;';
  if(AUTH.token) headerActions.appendChild(renderBellButton());
  const gearBtn = document.createElement('button');
  ...
  hero.className = 'hero-card';
  hero.innerHTML = `
    <div class="hero-number">${due}</div>
    <div class="hero-label">từ đến hạn ôn hôm nay</div>
  ...
  const label = document.createElement('div');
  label.className = 'section-label';
  label.textContent = `Từ vựng đã lưu (${DATA.vocab.length})`;
  main.appendChild(label);
...
function renderVocabList(list){
  ...
  if(words.length===0){
    list.appendChild(emptyState('🔤', q ? 'Không tìm thấy từ nào phù hợp.' : 'Chưa có từ vựng nào. Nhấn + để thêm từ đầu tiên.'));
    return;
  }
...
function renderVocabAdd(){
  ...
  function field(labelText, id, placeholder, value, isTextarea){
    ...
    if(!isTextarea) el.type = 'text'; else el.rows = 2;
    el.id = id;
  ...
  POS_QUICK_PICKS.forEach(p=>{
    const chip = document.createElement('button');
    chip.type = 'button';
    chip.className = 'chip' + (posEl.value===p ? ' active' : '');
    chip.textContent = p;
    chip.onclick = ()=>{
      posEl.value = p;
...
function renderDeleteVocabModal(){
  ...
  const overlay = document.createElement('div');
  overlay.className = 'modal-backdrop';
  overlay.onclick = (e)=>{ if(e.target===overlay){ deleteVocabId=null; render(); } };

  const card = document.createElement('div');
  card.className = 'modal-card';

  if(!w){ deleteVocabId = null; return overlay; }

  card.innerHTML = `
    <div class="modal-title display">Xoá từ này?</div>
    <p style="color:var(--ink-soft); font-size:14px; line-height:1.6; margin:0 0 4px;">
      Hành động này không thể hoàn tác.
    </p>
    <div style="background:var(--bg-elev); border:1px solid var(--line); border-radius:12px; padding:14px; margin-top:14px;">
      <div style="font-size:14px; font-weight:600; line-height:1.4;">${escapeHtml(w.word)}</div>
      <div style="font-size:13px; color:var(--ink-faint); margin-top:6px; line-height:1.4;">${escapeHtml(w.meaning)}</div>
    </div>
  `;
  ...
  cancelBtn.style.border = '1px solid var(--line)';
  cancelBtn.textContent = 'Huỷ';
  cancelBtn.onclick = ()=>{ deleteVocabId=null; render(); };

  const delBtn = document.createElement('button');
  delBtn.className = 'save-btn';
  delBtn.style.background = 'var(--coral)';
  delBtn.style.color = '#3a0d13';
  delBtn.textContent = 'Xoá từ';
  delBtn.onclick = async ()=>{
    const id = deleteVocabId;
    DATA.vocab = DATA.vocab.filter(x=>x.id!==id);
    deleteVocabId = null;
    await saveData();
    toast('Đã xoá từ vựng ✓');
    render();
  };
...
function startVocabReview(){
  vocabReviewQueue = dueVocab().sort((a,b)=>a.due-b.due);
  vocabReviewIdx = 0;
  vocabFlipped = false;
...
function renderVocabReview(){
  ...
  if(vocabReviewQueue.length===0 || vocabReviewIdx >= vocabReviewQueue.length){
    if(vocabReviewQueue.length>0 && !vocabSessionCompletionHandled){
      vocabSessionCompletionHandled = true;
      saveData();
    }
    const p = DATA.progress;
    const lvl = computeLevel(p.xp);
    wrap.innerHTML = `<div class="review-done">
      <div class="glyph">🎉</div>
      <h2 class="display">Xong hết rồi!</h2>
      <p>Bạn đã ôn hết từ vựng đến hạn. Quay lại vào ngày mai nhé.</p>
      ${vocabReviewQueue.length>0 ? `
      <div class="session-recap">
        <div class="session-recap-item"><span class="srv">+${vocabSessionXpEarned}</span><span class="srl">XP</span></div>
        <div class="session-recap-item"><span class="srv">🔥 ${p.streak}</span><span class="srl">ngày liên tục</span></div>
        <div class="session-recap-item"><span class="srv">Lv.${lvl.level}</span><span class="srl">${lvl.xpIntoLevel}/${lvl.xpForThisLevel} XP</span></div>
      </div>` : ''}
    </div>`;
  ...
  if(!vocabFlipped){
    ...
    const preview = {
      ...
      good: (()=>{ const c={...w}; grade(c,2); return c.interval; })(),
      easy: (()=>{ const c={...w}; grade(c,3); return c.interval; })(),
    };
    const row = document.createElement('div');
    row.className = 'grade-row';
    row.innerHTML = `
    ...
    row.querySelectorAll('.grade-btn').forEach((btn,i)=>{
      btn.onclick = async ()=>{
        ...
        grade(w, quality);
        recordXpAndStreak(quality);
        vocabSessionXpEarned += (XP_PER_GRADE[quality] || 0);
        await saveData();
        vocabReviewIdx += 1;
        vocabFlipped = false;
        render();
