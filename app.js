...
/* ---------------- ĐỌC FILE .DOCX THÀNH VĂN BẢN (giữ được công thức toán) ----------------
   ...
   chứ không phải chữ thường — nếu chỉ lấy text thô kiểu thư viện ngoài (mammoth) thì toàn
   bộ công thức sẽ bị BỎ TRẮNG. Hàm omlToText() dưới đây tự "đọc hiểu" cây đó và viết lại
   thành text thường (ví dụ phân số 5π/12 → "(5π)/(12)"), sát nghĩa gốc, để không mất dữ liệu. */
...
// Chuyển 1 node công thức toán (Word OMML) thành text thường, đệ quy. Chỉ xử lý các dạng
// hay gặp trong đề thi phổ thông: phân số, căn, số mũ/chỉ số, ngoặc, tổng/tích (Σ/Π gần đúng).
function omlToText(node){
  ...
  if(tag==='m:f'){
    ...
    return `(${num?omlContainerText(num):''})/(${den?omlContainerText(den):''})`;
  }
  if(tag==='m:rad'){
    ...
    return (degTxt && degTxt!=='2') ? `${degTxt}√(${inner})` : `√(${inner})`;
  }
  if(tag==='m:sSup'){
  ...
  if(tag==='m:d'){
    ...
    if(dPr){
      ...
      if(begEl){ const v = begEl.getAttribute('m:val'); if(v) beg = v; }
      if(endEl){ const v = endEl.getAttribute('m:val'); if(v) end = v; }
    }
    return beg + domFindChildren(node,'m:e').map(omlContainerText).join(', ') + end;
  }
  if(tag==='m:nary'){
    ...
    return `${chr}${sub?'_('+omlContainerText(sub)+')':''}${sup?'^('+omlContainerText(sup)+')':''}(${e?omlContainerText(e):''})`;
  }
  if(tag==='m:bar'){ const e = domFindChild(node,'m:e'); return `${e?omlContainerText(e):''}‾`; }
...
// Text của 1 đoạn <w:p>, thay riêng phần công thức toán bằng omlToText().
function docxParagraphText(pNode){
  let text = '';
  function walk(n){
    if(n.nodeType!==1) return;
    if(n.tagName==='m:oMath'){ text += omlToText(n); return; }
